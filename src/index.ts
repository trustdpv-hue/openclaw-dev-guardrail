...
const GUARDRAIL_REASON =
  "🛑 Dev Workflow Guardrail: Blocked direct production command. Follow the workflow: Modify locally → Check → Get approval → Deploy.";

const DEPLOY_SUGGESTION =
  "If you need to deploy, first make changes locally, test them, then get approval before deploying to production.";
...
export default definePluginEntry({
  ...
  register(api) {
    const config: GuardrailConfig = (api.pluginConfig ?? {}) as GuardrailConfig;
    ...
    api.on(
      ...
      async (event) => {
        ...
        const targetsProduction =
          /\bssh\b/.test(fullCommand) ||
          /\bscp\b/.test(fullCommand) ||
          /\brsync\b/.test(fullCommand) ||
          productionIps.some((ip: string) => fullCommand.includes(ip)) ||
          productionHosts.some((host: string) => {
        ...
        if (isNonSshCommand) {
          ...
          const isSafeHttpRequest =
            /\bcurl\b/.test(fullCommand) &&
            !/\b(-X\s+(POST|PUT|DELETE|PATCH)|--data|--upload|-d\s)/.test(
              fullCommand
            );
          const isSafeWgetRequest =
            /\bwget\b/.test(fullCommand) && /\b(--spider|-q)\b/.test(fullCommand);
        ...
        const hasDangerousChaining = DANGEROUS_CHAINING.some((p) =>
          p.test(fullCommand)
        );
        ...
        const hasWriteRedirect =
          /\b(>>?)\b/.test(fullCommand) && !/\b2>>/.test(fullCommand);
        if (hasWriteRedirect) {
          return {
            block: true,
        ...
        const isSshReadOnly = sshReadOnlyCommands.some((pattern) =>
